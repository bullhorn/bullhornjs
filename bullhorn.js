...
(function(root) {
	...
	Bullhorn.prototype.authenticate = function(oauth) {
		...
		function authenticated(data) {
			...
			Bullhorn.entitlements = [];
			data.userInformation.entitlements.each(function(entity, key) {
				entity.each(function(permission, i) {
					Bullhorn.entitlements.push(key + '.' + permission);
				});
			});

			var settings = data.userInformation.settings;

			settings.userEntitlements.forEach(function(permission, i) {
				Bullhorn.entitlements.push(permission);
			});

			if(settings.allowInlineEditing) Bullhorn.entitlements.push('allowInlineEditing');

			if(settings.allowColumnCustomization) Bullhorn.entitlements.push('allowColumnCustomization');

			Candidate.label = settings.entityTitleCandidate;
			Candidate.color = settings.headingBGColorCandidate;
			Candidate.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsCandidate);
			Candidate.previewFields = cleanUnsafeFields(settings.listExpandedLeftCandidate);

			Contact.label = settings.entityTitleClientContact;
			Contact.color = settings.headingBGColorClientContact;
			Contact.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsClientContact);
			Contact.previewFields = cleanUnsafeFields(settings.listExpandedLeftClientContact);

			Company.label = settings.entityTitleClientCorporation;
			Company.color = settings.headingBGColorClientCorporation;
			Company.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsClientCorporation);
			Company.previewFields = cleanUnsafeFields(settings.listExpandedLeftClientCorporation);

			Job.label = settings.entityTitleJobOrder;
			Job.color = settings.headingBGColorJobOrder;
			Job.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsJobOrder);
			Job.previewFields = cleanUnsafeFields(settings.listExpandedLeftJobOrder);

			Placement.label = settings.entityTitlePlacement;
			Placement.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsPlacement);
			Placement.previewFields = cleanUnsafeFields(settings.listExpandedLeftPlacement);

			Submission.label = settings.entityTitleJobSubmission;
			Submission.defaultColumns = ['id','candidate','jobOrder','sendingUser','status','source','payRate','salary','billRate']; //cleanUnsafeFields(settings.defaultListColumnsJobSubmission);
			Submission.previewFields = ['id','candidate','jobOrder','sendingUser','status','source','payRate','salary','billRate']; //cleanUnsafeFields(settings.listExpandedLeftJobSubmission);

			Lead.label = settings.entityTitleLead;
			Lead.color = settings.headingBGColorLead;
			Lead.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsLead);
			Lead.previewFields = cleanUnsafeFields(settings.listExpandedLeftLead);

			Opportunity.label = settings.entityTitleOpportunity;
			Opportunity.color = settings.headingBGColorOpportunity;
			Opportunity.defaultColumns = cleanUnsafeFields(settings.defaultListColumnsOpportunity);
			Opportunity.previewFields = cleanUnsafeFields(settings.listExpandedLeftOpportunity);


			Bullhorn.user = data.userInformation.userData;
			Bullhorn.user.corporation = {
				id: settings.corporationId,
				name: settings.corporationName
			};
			Bullhorn.user.privateLabel = settings.corpPrivateLabelId;
			//Bullhorn.user.dateFormat = ( settings.locale && settings.locale == "en_GB") ? 'd/M/yyyy' : 'M/d/yyyy';
			//Bullhorn.user.dateTimeFormat = ( settings.locale && settings.locale == "en_GB") ? 'd/M/yyyy h:mm a' : 'M/d/yyyy h:mm a'
			Bullhorn.settings = settings;
